...
Revision ID: 0030_job_queue_rls_roles
Revises: 0029_auth_rate_limits
...
revision = "0030_job_queue_rls_roles"
down_revision = "0029_auth_rate_limits"
branch_labels = None
depends_on = None
...
_HETZNER_PROVIDER_DEFAULTS = (
    "ALTER TABLE IF EXISTS provisioning_runs ALTER COLUMN external_provider SET DEFAULT 'hetzner'",
    "ALTER TABLE IF EXISTS control_rollouts ALTER COLUMN external_provider SET DEFAULT 'hetzner'",
)
...
def upgrade() -> None:
    ...
    # Provisioning and pull-path rollouts are Hetzner-only.  Existing database
    # defaults predate that architecture and would otherwise label direct SQL
    # inserts as GitHub Actions even though no workflow dispatcher remains.
    for statement in _HETZNER_PROVIDER_DEFAULTS:
