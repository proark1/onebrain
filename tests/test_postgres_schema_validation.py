...
def _job_leases_module():
    return _load_migration_module("0026_job_leases.py", "job_leases_migration")


def _ai_agent_run_leases_module():
    return _load_migration_module("0027_ai_agent_run_leases.py", "ai_agent_run_leases_migration")


def _customer_teardown_module():
    return _load_migration_module("0028_customer_teardown_protocol.py", "customer_teardown_migration")


def _auth_rate_limits_module():
    return _load_migration_module("0029_auth_rate_limits.py", "auth_rate_limits_migration")
...
def _accounting_foundations_module():
    return _load_migration_module(
        "0036_accounting_foundations.py", "accounting_foundations_migration"
    )
...
def test_accounting_foundations_migration_is_scoped_and_forced_rls():
    migration = _accounting_foundations_module()
    source = (
        Path(__file__).resolve().parents[1]
        / "migrations" / "versions" / "0036_accounting_foundations.py"
    ).read_text()
    ...
    assert "accounting_line_items_document_scope_fk" in source
    assert "proposed_account" in source and "confirmed_account" in source
...
def test_job_leases_migration_precedes_the_current_head():
    ...
    assert migration.revision == "0026_job_leases"
    assert migration.down_revision == "0025_provisioning_module_selection"
...
def test_recovery_auth_job_queue_and_drive_migrations_form_one_chain():
    ...
    assert teardown.revision == "0028_customer_teardown_protocol"
...
def test_start_rollout_insert_persists_fleet_rollout_id():
    ...
    class InsertCursor:
        ...
        def fetchone(self):
            ...
            return (
                ...
                "pending", by_column["external_provider"], "", "", "", {}, None, None,
    ...
    assert dict(zip(columns, cursor.params))["external_provider"] == "hetzner"
    assert returned.fleet_rollout_id == "fr_1"  # readable back on the postgres path
    assert returned.external_provider == "hetzner"
