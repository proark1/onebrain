...
def _migration_module():
    path = Path(__file__).resolve().parents[1] / "migrations" / "versions" / "0029_job_queue_rls_roles.py"
...
def test_job_queue_migration_is_head_successor_with_role_scoped_policies():
    ...
    assert migration.revision == "0029_job_queue_rls_roles"
    assert migration.down_revision == "0028_auth_rate_limits"
