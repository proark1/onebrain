...
JSON-only operator state store. The current additive hardening sequence is:

- `0025_job_leases`: lease token and expiry fields plus claim index for durable
  background jobs;
- `0026_ai_agent_run_leases`: fenced leases and heartbeat state for direct AI
  Employee turns;
- `0027_customer_teardown_protocol`: a record-only two-person review table;
  it has no deletion executor; and
- `0028_auth_rate_limits`: shared fixed-window counters with HMAC-hashed
  subjects for all API replicas; and
- `0029_job_queue_rls_roles`: forced RLS and separate restricted PostgreSQL
...
4. Run `alembic upgrade head` before adding new API or worker replicas.
   Revision `0029_job_queue_rls_roles` creates the queue policies, grants the
...
`0029_job_queue_rls_roles` is required for production queue use. The app role
...
2. Run `alembic upgrade head` as the database owner with both
   `ONEBRAIN_POSTGRES_*_ROLE` names set. Do not run `0029` as either restricted
   login.
...
Migration `0028_auth_rate_limits` adds `auth_rate_limits`, which production
...
Migration `0026_ai_agent_run_leases` gives direct AI Employee turns the same
...
Migration `0027_customer_teardown_protocol` is an audit/review protocol, not a
